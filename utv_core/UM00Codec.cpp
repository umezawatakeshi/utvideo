...
size_t CUM00Codec::EncodeFrame(void *pOutput, bool *pbKeyFrame, const void *pInput)
{
	...
	// XXX ÉRÉsÅ[ÉtÉåÅ[ÉÄÇ…Ç»ÇÈÇ©Ç«Ç§Ç©ÇÃîªíËÇ™óvÇÈÇ™å„Ç≈é¿ëï
...
int CUM00Codec::InternalEncodeBegin(utvf_t infmt, unsigned int width, unsigned int height, size_t cbGrossWidth)
{
	...
	for (int i = 0; i < GetNumPlanes(); i++)
	{
		...
		{
			// XXX í[êîÇçló∂Ç…ì¸ÇÍÇƒÇ»Ç¢ÇÃÇ≈å„Ç≈íºÇ∑
			m_pPackedStream[i][j] = (uint8_t*)malloc(m_cbPlaneSize[i] / m_dwDivideCount);
			m_pControlStream[i][j] = (uint8_t*)malloc(m_cbPlaneSize[i] / 64 * 3 / m_dwDivideCount);
		}
	}
...
{
	return ROUNDUP(width, 128) * height * GetRealBitCount() / 8 + 4096; // +4096 ÇÕÇ«ÇÒÇ‘ÇËä®íËÅB
}
