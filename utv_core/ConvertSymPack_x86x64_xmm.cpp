...
static inline void tuned_ConvertRGBXToULRX_Pack8SymAfterPredictPlanarGradient8(uint8_t *pGPacked, size_t *cbGPacked, uint8_t *pGControl, uint8_t *pBPacked, size_t *cbBPacked, uint8_t *pBControl, uint8_t *pRPacked, size_t *cbRPacked, uint8_t *pRControl, uint8_t *pAPacked, size_t *cbAPacked, uint8_t *pAControl, const uint8_t *pSrcBegin, const uint8_t *pSrcEnd, size_t cbWidth, ssize_t scbStride)
{
...

	{
		...
		{
			auto planar0 = tuned_ConvertPackedRGBXToPlanarElement<F, __m128i, T, false>(pp);
			auto planar1 = tuned_ConvertPackedRGBXToPlanarElement<F, __m128i, T, false>(pp + T::BYPP * 16);
		...
		if (pp < p + cbWidth)
		{
			...
			for (; pp < p + cbWidth && n < 32; pp += T::BYPP, ++n)
			{
				uint8_t gg = pp[T::G];
				gps.b[n] = gg - gprevb;
	...
	for (auto p = pSrcBegin + scbStride; p != pSrcEnd; p += scbStride)
	{
		...
		for (; pp <= p + cbWidth - T::BYPP * 32; pp += T::BYPP * 32)
		{
			auto planar0 = tuned_ConvertPackedRGBXToPlanarElement<F, __m128i, T, false>(pp, scbStride);
			auto planar1 = tuned_ConvertPackedRGBXToPlanarElement<F, __m128i, T, false>(pp + T::BYPP * 16, scbStride);
...
static inline void tuned_ConvertULRXToRGBX_Unpack8SymAndRestorePlanarGradient8Impl(uint8_t *pDstBegin, uint8_t *pDstEnd, const uint8_t *pGPacked, const uint8_t *pGControl, const uint8_t *pBPacked, const uint8_t *pBControl, const uint8_t *pRPacked, const uint8_t *pRControl, const uint8_t *pAPacked, const uint8_t *pAControl, size_t cbWidth, ssize_t scbStride)
{
...

	{
		...
		for (; pp <= p + cbWidth - T::BYPP * 16; pp += T::BYPP * 16, lb += T::BYPP * 16)
		{
			...
			if (!NTSTORE)
				tuned_ConvertPlanarRGBXToPackedElement<F, __m128i, T, false>(pp, gvalue.v0, bvalue.v0, rvalue.v0, A ? avalue.v0 : _mm_set1_epi8((char)0xff));
			else
				tuned_ConvertPlanarRGBXToPackedElement<F, __m128i, T, false, NTSTORE>(pp, lb, gvalue.v0, bvalue.v0, rvalue.v0, A ? avalue.v0 : _mm_set1_epi8((char)0xff));
	...
	for (auto p = pDstBegin + scbStride; p != pDstEnd; p += scbStride)
	{
		...
		for (; pp <= p + cbWidth - T::BYPP * 16; pp += T::BYPP * 16, lb += T::BYPP * 16)
		{
			...
			if (!NTSTORE)
				tuned_ConvertPlanarRGBXToPackedElement<F, __m128i, T, false>(pp, gvalue.v0, bvalue.v0, rvalue.v0, A ? avalue.v0 : _mm_set1_epi8(0), scbStride);
			else
				tuned_ConvertPlanarRGBXToPackedElement<F, __m128i, T, false, NTSTORE>(pp, gvalue.v0, bvalue.v0, rvalue.v0, A ? avalue.v0 : _mm_set1_epi8(0), lb);
...
void tuned_ConvertPackedYUV422ToULY2_Pack8SymAfterPredictPlanarGradient8(uint8_t *pYPacked, size_t *cbYPacked, uint8_t *pYControl, uint8_t *pUPacked, size_t *cbUPacked, uint8_t *pUControl, uint8_t *pVPacked, size_t *cbVPacked, uint8_t *pVControl, const uint8_t *pSrcBegin, const uint8_t *pSrcEnd, size_t cbWidth, ssize_t scbStride)
{
...

	{
		...
		{
			auto planar0 = tuned_ConvertPackedYUV422ToPlanarElement<F, __m128i, T>(pp);
			auto planar1 = tuned_ConvertPackedYUV422ToPlanarElement<F, __m128i, T>(pp + 64);
	...
	for (auto p = pSrcBegin + scbStride; p != pSrcEnd; p += scbStride)
	{
		...
		for (; pp <= p + cbWidth - 128; pp += 128)
		{
			auto planar0 = tuned_ConvertPackedYUV422ToPlanarElement<F, __m128i, T>(pp, scbStride);
			auto planar1 = tuned_ConvertPackedYUV422ToPlanarElement<F, __m128i, T>(pp + 64, scbStride);
