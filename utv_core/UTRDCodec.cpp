	...
	for (int nPlaneIndex = 0; nPlaneIndex < 3; nPlaneIndex++)
	{
		...
		for (unsigned y = 0; y < m_nHeight; y += 1)
		{
			for (unsigned x = 0; x < m_nWidth; x += 8)
			{
				...
				{
					auto signs = w & 0x8080808080808080ULL; // ïÑçÜbitÇæÇØÇíäèo
					signs = (signs << 1) - (signs >> 7); //ïÑçÜbitÇbyteÇ…ìWäJ
					...
					uint64_t offadj = (w & 0x8080808080808080ULL) << 1; // ïÑçÜÇæÇØï™Ç©ÇÍÇŒÇ¢Ç¢ÇÃÇ≈ offset Ç©ÇÁåvéZÇ∑ÇÈïKóvÇÕÇ»Ç¢ / ç≈èâÇÃï˚ÇÃèàóùÇ∆ìØÇ∂éÆÇ»ÇÃÇ≈ÉRÉìÉpÉCÉâÇ™å´ÇØÇÍÇŒégÇ¢Ç‹ÇÌÇµÇƒÇ≠ÇÍÇÈÇÕÇ∏
					w += offset;
					w -= offadj;
