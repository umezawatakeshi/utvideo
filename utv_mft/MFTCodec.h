...
protected:
	...
	size_t m_cbInStride, m_cbOutStride;
	bool m_bStreamBegin;
...
public:
	...
	static void LOGPRINT_IMFMT_THIS(CMFTCodec *_this, const char *prefix, IMFMediaType *pType)
	{
		LockAttr lockattr(pType);

		GUID guidMajorType, guidSubtype;
		UINT32 nFrameWidth, nFrameHeight;
		INT32 scbStrideFromType;
		LONG scbStrideForBih;

		OLECHAR szMajorType[48], szSubtype[48];
		char szFrameSize[32], szStrideFromType[16], szStrideForBih[16];

		if (SUCCEEDED(pType->GetGUID(MF_MT_MAJOR_TYPE, &guidMajorType)))
			StringFromGUID2(guidMajorType, szMajorType, _countof(szMajorType));
		else
			wsprintf(szMajorType, L"(none)");

		if (SUCCEEDED(pType->GetGUID(MF_MT_SUBTYPE, &guidSubtype)))
			StringFromGUID2(guidSubtype, szSubtype, _countof(szSubtype));
		else
			wsprintf(szSubtype, L"(none)");

		if (SUCCEEDED(MFGetAttributeSize(pType, MF_MT_FRAME_SIZE, &nFrameWidth, &nFrameHeight)))
			sprintf(szFrameSize, "%dx%d", nFrameWidth, nFrameHeight);
		else
			strcpy(szFrameSize, "(none)");

		if (SUCCEEDED(pType->GetUINT32(MF_MT_DEFAULT_STRIDE, (UINT32*)&scbStrideFromType)))
			sprintf(szStrideFromType, "%d", scbStrideFromType);
		else
			strcpy(szStrideFromType, "(none)");

		if (SUCCEEDED(MFGetStrideForBitmapInfoHeader(guidSubtype.Data1, nFrameWidth, &scbStrideForBih)))
			sprintf(szStrideForBih, "%d", scbStrideForBih);
		else
			strcpy(szStrideForBih, "(none)");

		LOGPRINTF("%p %s: MAJOR_TYPE=%S, SUBTYPE=%S, FRAME_SIZE=%s, DEFAULT_STRIDE=%s MFGetStrideForBitmapInfoHeader=%s",
			(_this), (prefix), szMajorType, szSubtype, szFrameSize, szStrideFromType, szStrideForBih);
	}
	...
	HRESULT STDMETHODCALLTYPE GetInputAvailableType(DWORD dwInputStreamID, DWORD dwTypeIndex, IMFMediaType **ppType)
	{
		...
		for (;; putvf++)
		{
			...
			if (UtVideoFormatToMediaFoundationFormat(&subtype, &bBottomup, *putvf) != 0)
				continue;
			if (dwTypeIndex == 0)
				break;
			dwTypeIndex--;
		...
		if (IsLogWriterInitializedOrDebugBuild())
		{
			LOGPRINT_IMFMT_THIS(this, " returned", *ppType);
		}
	...
	HRESULT STDMETHODCALLTYPE GetOutputAvailableType(DWORD dwOutputStreamID, DWORD dwTypeIndex, IMFMediaType **ppType)
	{
		...
		if (m_pInputMediaType != NULL)
		{
			...
			/*
			 * RGB で MF_MT_DEFAULT_STRIDE を省略した場合、
			 * topdown か bottomup かがマイクロソフトのコードですら混乱しているので、
			 * 必ず付けて返すようにする。
			 *
			 * できれば GetInputAvailableType でもそうしたいが、
			 * width/height が確定していないので stride を計算できない。
			 */
			if (IsEqualGUID(subtype, MFVideoFormat_RGB24) || IsEqualGUID(subtype, MFVideoFormat_RGB32) || IsEqualGUID(subtype, MFVideoFormat_ARGB32))
			{
				int bypp;
				if (IsEqualGUID(subtype, MFVideoFormat_RGB24))
					...
					bypp = 4;
				int width = (m_nFrameWidth * bypp + 3) / 4 * 4;
				(*ppType)->SetUINT32(MF_MT_DEFAULT_STRIDE, width * (bBottomup ? -1 : 1));
			}
		}
	...
	HRESULT STDMETHODCALLTYPE SetInputType(DWORD dwInputStreamID, IMFMediaType *pType, DWORD dwFlags)
	{
		if (IsLogWriterInitializedOrDebugBuild())
		{
			...
			if (pType != NULL)
				LOGPRINT_IMFMT_THIS(this, " pType", pType);
		}
		...
		if (SUCCEEDED(pType->GetUINT32(MF_MT_DEFAULT_STRIDE, (UINT32*)&scbStride)) ||
			SUCCEEDED(MFGetStrideForBitmapInfoHeader(guidNewSubtype.Data1, nFrameWidth, (LONG*)&scbStride)))
		{
			bBottomup = scbStride < 0;
		}
		else
		{
			/*
			 * MFGetStrideForBitmapInfoHeader が値を返してくれないようなフォーマットで UtVideo がサポートしているものは全て topdown である。
			 * この時、 stride はデフォルトの物をコーデック実装の内部で計算して使うようにする。
			 */
			scbStride = 0;
			bBottomup = false;
		}
		if (MediaFoundationFormatToUtVideoFormat(&infmt, guidNewSubtype, bBottomup) != 0)
			return MF_E_INVALIDMEDIATYPE;
		...
		m_infmt = infmt;
		m_cbInStride = scbStride != 0 ? abs(scbStride) : CBGROSSWIDTH_WINDOWS;
	...
	HRESULT STDMETHODCALLTYPE SetOutputType(DWORD dwOutputStreamID, IMFMediaType *pType, DWORD dwFlags)
	{
		...
		if (MediaFoundationFormatToUtVideoFormat(&outfmt, guidNewSubtype, bBottomup) != 0)
			return MF_E_INVALIDMEDIATYPE;
		...
		m_outfmt = outfmt;
		m_cbOutStride = scbStride != 0 ? abs(scbStride) : CBGROSSWIDTH_WINDOWS;
	...
	{
		// 再生中は BeginStream() が呼ばれ続けるのでログを出せない。
