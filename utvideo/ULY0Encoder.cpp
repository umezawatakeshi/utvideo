...
const CULY0Encoder::INPUTFORMAT CULY0Encoder::m_infmts[1] = {
	{ FCC('YV12'), 12 },
};
...
void CULY0Encoder::CalcPlaneSizes(const BITMAPINFOHEADER *pbihIn)
{
	m_dwPlaneStride[0]      = pbihIn->biWidth;
	m_dwPlaneMacroStride[0] = m_dwPlaneStride[0] * 2;
	m_dwPlaneSize[0]        = pbihIn->biWidth * pbihIn->biWidth;

	m_dwPlaneStride[1]      = pbihIn->biWidth / 2;
	m_dwPlaneMacroStride[1] = m_dwPlaneStride[1];
	m_dwPlaneSize[1]        = pbihIn->biWidth * pbihIn->biWidth / 4;
